...
- **Brand color / theming**: `#4F39F6` (purple) is *not yet confirmed* as final. The user will provide a proper style guide/brand color spec separately — don't treat the current purple as locked in, and don't invest in wiring runtime tenant-color theming (see the root `PROJECT_STATUS.md`'s "wms-app Frontend Tech Debt" section) until that spec arrives.
